...
class Settings {
	...
	private function set_options_prefix( $options_prefix = '' ) {
		if ( empty( $opts_prefix ) ) {
			$opts_prefix = str_replace( '-', '_', 'tec-labs-advanced-ical-export-v2' ); // The text domain.
		}
	...
	public function add_settings() {
		$fields = [
			...
			'a_setting' => [ // TODO: Change setting.
				'type'            => 'text',
				'label'           => esc_html__( 'Example setting', 'tec-labs-advanced-ical-export-v2' ),
				'tooltip'         => sprintf( esc_html__( 'Example setting description. Enter your custom URL, including "http://" or "https://", for example %s.', 'tec-labs-advanced-ical-export-v2' ), '<code>https://demo.theeventscalendar.com/</code>' ),
	...
	private function get_example_intro_text() {
		$result = '<h3>' . esc_html_x( 'Example Extension Setup', 'Settings header', 'tec-labs-advanced-ical-export-v2' ) . '</h3>';
		$result .= '<div style="margin-left: 20px;">';
		$result .= '<p>';
		$result .= esc_html_x( 'Some text here about this settings section.', 'Setting section description', 'tec-labs-advanced-ical-export-v2' );
		$result .= '</p>';
		$result .= '</div>';
